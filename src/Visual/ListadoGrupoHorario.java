...
public class ListadoGrupoHorario extends JDialog {
	...
	public static void main(String[] args) {
		try {
			...
			ListadoGrupoHorario dialog = new ListadoGrupoHorario(academico);
	 ...
	 * Create the dialog.
	 */
	public ListadoGrupoHorario(Academico academico) {
		this.academico = academico;
		setTitle("Listado horario grupo(s)");
		...
		contentPanel.setLayout(null);
		{
			...
			panel.setLayout(new BorderLayout(0, 0));
			{
				...
				{
					String[] columnas = {"Código periodo Aca.","Código asignatura","Número grupo","Día Semana","Hora inicial","Hora final"};
		...
		textCodPeriodoAcad.setBounds(182, 21, 75, 20);
		...
		
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				btnAgregar.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						AgregarGrupoHorario agregarGrupoHorario = new AgregarGrupoHorario(academico);
						agregarGrupoHorario.setModal(true);
						agregarGrupoHorario.setVisible(true);
			...
			}
			{
				...
				btnmModificar.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						ModificarGrupoHorario modificarGrupoHorario = new ModificarGrupoHorario(academico,(String) table.getModel().getValueAt(selectedRow, 0), (String) table.getModel().getValueAt(selectedRow, 1),(String) table.getModel().getValueAt(selectedRow, 2),
								Integer.parseInt((String)table.getModel().getValueAt(selectedRow, 3)),(String) table.getModel().getValueAt(selectedRow, 4));
						modificarGrupoHorario.setModal(true);
						modificarGrupoHorario.setVisible(true);
			...
			}
			{
				...
				EliminarButton.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						int option = JOptionPane.showConfirmDialog(null, "¿Esta seguro de que desea eliminar el grupo horario? ", "Eliminar", JOptionPane.WARNING_MESSAGE);
						if(option == 0) {
							try {
								ListadoGrupoHorario.this.academico.getGrupoHorario().delete((String) table.getModel().getValueAt(selectedRow, 0),(String) table.getModel().getValueAt(selectedRow, 1),(String) table.getModel().getValueAt(selectedRow, 2),
										Integer.valueOf((String) table.getModel().getValueAt(selectedRow, 3)),(String) table.getModel().getValueAt(selectedRow, 4));
								JOptionPane.showMessageDialog(null, "Operación Realizada", "Información", JOptionPane.INFORMATION_MESSAGE);

							} catch (SQLException e1) {
								// TODO Auto-generated catch block
								JOptionPane.showMessageDialog(null, "Error al eliminar la grupo horario", "Información", JOptionPane.ERROR_MESSAGE);
								e1.printStackTrace();
							}
						}
						try {
							...
							// TODO Auto-generated catch block
							JOptionPane.showMessageDialog(null, "Error al cargar los grupos horario", "Información", JOptionPane.ERROR_MESSAGE);
							e1.printStackTrace();
						}
						btnmModificar.setEnabled(false);
						EliminarButton.setEnabled(false);
	...
	public static void loadListado(Academico academico) throws SQLException {
		...
		ArrayList<ArrayList<String> > Datatabla = academico.getGrupoHorario().select();
	...
	public static void loadListadoFiltrado(Academico academico,String codPeriodoAcad, String codAsignatura,String numGrupo) throws SQLException {
		model.setRowCount(0);
		fila = new Object[model.getColumnCount()];
		ArrayList<ArrayList<String> > Datatabla = academico.getGrupoHorario().selectWhere(codPeriodoAcad, codAsignatura, numGrupo);
